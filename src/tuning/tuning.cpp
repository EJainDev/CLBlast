...
// Constants holding start and end strings for terminal-output in colour
  ...
  for (auto& o : defaults.options) {
    ...
    if (o == kArgN) {
      args.n = GetArgument(command_line_args, help, kArgN, defaults.default_n);
    }
    if (o == kArgK) {
      args.k = GetArgument(command_line_args, help, kArgK, defaults.default_k);
    }
    if (o == kArgChannels) {
      args.channels = GetArgument(command_line_args, help, kArgChannels, defaults.channels);
    }
    if (o == kArgHeight) {
      args.height = GetArgument(command_line_args, help, kArgHeight, defaults.height);
    }
    if (o == kArgWidth) {
      args.width = GetArgument(command_line_args, help, kArgWidth, defaults.width);
    }
    if (o == kArgKernelH) {
      args.kernel_h = GetArgument(command_line_args, help, kArgKernelH, defaults.kernel_h);
    }
    if (o == kArgKernelW) {
      args.kernel_w = GetArgument(command_line_args, help, kArgKernelW, defaults.kernel_w);
    }
    if (o == kArgNumKernels) {
      args.num_kernels = GetArgument(command_line_args, help, kArgNumKernels, defaults.num_kernels);
    }
    if (o == kArgAlpha) {
      args.alpha = GetArgument(command_line_args, help, kArgAlpha, GetScalar<T>());
    }
    if (o == kArgBeta) {
      args.beta = GetArgument(command_line_args, help, kArgBeta, GetScalar<T>());
    }
    if (o == kArgBatchCount) {
      args.batch_count = GetArgument(command_line_args, help, kArgBatchCount, defaults.default_batch_count);
    }
  }
  ...
  const auto buffer_sizes = std::vector<size_t>{settings.size_x + kCanarySize, settings.size_y + kCanarySize,
                                                ...
                                                settings.size_c + kCanarySize, settings.size_temp + kCanarySize};
  ...
  printf("\n");
  printf("|   ID | total |");
  for (auto i = size_t{0}; i < settings.parameters.size() - 1; ++i) {
    printf("     ");
  }
  ...
  try {
    ...
    printf("      - |");
    if (time_ms == -1.0) {
      throw std::runtime_error("Error in reference implementation");
    }
  ...
  for (auto config_id = size_t{0}; config_id < configurations.size(); ++config_id) {
    try {
      ...
      const auto status_code = DispatchExceptionCatchAll(true);
      printf("  %scompilation error: %5d%s     |", kPrintError.c_str(), static_cast<int>(status_code),
             kPrintEnd.c_str());
      printf("      - |                 - | <-- skipping\n");
    } catch (...) {
      const auto status_code = DispatchExceptionCatchAll(true);
      if (status_code != StatusCode::kUnknownError) {
        printf("   %serror code %d%s |", kPrintError.c_str(), static_cast<int>(status_code), kPrintEnd.c_str());
      }
      printf(" <-- skipping\n");
    }
  }
  ...
  for (const auto& result : results) {
    average_ms += result.score;
  }
  average_ms /= results.size();
  printf("\n");
  printf("* Got average result of %.2lf ms", average_ms);
  printf(": %.1lf %s\n", settings.metric_amount / (average_ms * 1.0e6), settings.performance_unit.c_str());
  ...
  for (const auto& config : best_configuration->config) {
    ...
    if (i < best_configuration->config.size() - 1) {
      best_string += " ";
    }
    ++i;
  }
...
template void Tuner<double2>(int argc, char* argv[], const int V, GetTunerDefaultsFunc GetTunerDefaults,
                             ...
                             SetArgumentsFunc<double2> SetArguments);
